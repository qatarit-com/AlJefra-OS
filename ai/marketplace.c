...
#ifdef MARKETPLACE_USE_TLS
static ktls_conn_t g_tls_conn;
static int g_use_tls;
#endif

/* ── Transport abstraction: TLS or plain TCP ── */
...
static hal_status_t mkt_connect(mkt_conn_t *c, uint32_t ip, uint16_t port)
{
#ifdef MARKETPLACE_USE_TLS
    if (g_use_tls) {
        c->is_tls = 1;
        return ktls_connect(&c->tls, ip, port, MARKETPLACE_HOST);
    }
    c->is_tls = 0;
#endif
    return tcp_connect(&c->tcp, ip, port);
}

static int32_t mkt_send(mkt_conn_t *c, const void *data, uint32_t len)
{
#ifdef MARKETPLACE_USE_TLS
    if (c->is_tls) return ktls_send(&c->tls, data, len);
#endif
    return tcp_send(&c->tcp, data, len);
}

static int32_t mkt_recv(mkt_conn_t *c, void *buf, uint32_t max_len, uint32_t timeout_ms)
{
#ifdef MARKETPLACE_USE_TLS
    if (c->is_tls) return ktls_recv(&c->tls, buf, max_len);
#endif
    return tcp_recv(&c->tcp, buf, max_len, timeout_ms);
}

static void mkt_close(mkt_conn_t *c)
{
#ifdef MARKETPLACE_USE_TLS
    if (c->is_tls) { ktls_close(&c->tls); return; }
#endif
    tcp_close(&c->tcp);
}
...
static int32_t parse_content_length(const char *response, uint32_t len)
{
    const char *key = "Content-Length:";
    uint32_t klen = 15;
    for (uint32_t i = 0; i + klen < len; i++) {
        if (response[i] != 'C' && response[i] != 'c') continue;
        int match = 1;
        ...
        uint32_t j = i + klen;
        while (j < len && response[j] == ' ') j++;
        /* Parse number */
        int32_t val = 0;
        while (j < len && response[j] >= '0' && response[j] <= '9') {
            val = val * 10 + (response[j] - '0');
            j++;
        }
        return val;
    }
...
#ifdef MARKETPLACE_USE_TLS
    /* Production: TLS connection to api.aljefra.com:443 */
    g_use_tls = 1;
    ktls_init();
    hal_status_t rc = ktls_connect(&g_tls_conn, server_ip, MARKETPLACE_PORT,
                                    MARKETPLACE_HOST);
    if (rc != HAL_OK) {
        hal_console_puts("[marketplace] TLS connection failed\n");
        g_connected = 0;
        return rc;
    }
    /* Store the underlying TCP info for per-request connections */
    g_tcp_conn.remote_ip = server_ip;
    g_connected = 1;
    hal_console_puts("[marketplace] Connected (HTTPS/TLS)\n");
#else
...
hal_status_t marketplace_send_manifest(const hardware_manifest_t *manifest)
{
    ...
    /* Use a fresh connection for manifest POST */
    mkt_conn_t mconn;
    hal_status_t mrc = mkt_connect(&mconn, g_tcp_conn.remote_ip, MARKETPLACE_PORT);
    if (mrc != HAL_OK) {
        hal_console_puts("[marketplace] Connection failed for manifest\n");
        return HAL_ERROR;
    }

    /* Send request */
    int32_t sent = mkt_send(&mconn, request, (uint32_t)req_len);
    if (sent < 0) {
        mkt_close(&mconn);
        hal_console_puts("[marketplace] Send failed\n");
        return HAL_ERROR;
    }

    /* Receive response */
    char response[8192];
    int32_t resp_len = mkt_recv(&mconn, response, sizeof(response) - 1, 10000);
    mkt_close(&mconn);
    if (resp_len <= 0) {
...
/* Maximum driver package size: 1 MB */
#define MAX_DRIVER_SIZE  (1024 * 1024)
                                     ...
                                     void **data, uint64_t *size)
{
    ...
    *p = '\0';
    ...
    /* Need a fresh connection for each request (Connection: close) */
    mkt_conn_t drv_conn;
    hal_status_t rc = mkt_connect(&drv_conn, g_tcp_conn.remote_ip, MARKETPLACE_PORT);
    if (rc != HAL_OK) {
        hal_console_puts("[marketplace] Connect failed for driver download\n");
        return HAL_ERROR;
    }
    ...
    int32_t sent = mkt_send(&drv_conn, request, (uint32_t)req_len);
    if (sent < 0) {
        mkt_close(&drv_conn);
        return HAL_ERROR;
    }

    /* ── Streaming receive: read headers first, then stream body ── */

    /* Receive initial chunk (HTTP headers + start of body) */
    char hdr_buf[4096];
    int32_t hdr_len = mkt_recv(&drv_conn, hdr_buf, sizeof(hdr_buf) - 1, 10000);
    if (hdr_len <= 0) {
        mkt_close(&drv_conn);
        return HAL_TIMEOUT;
    }
    ...
    /* Check status */
    int status = parse_http_status(hdr_buf, (uint32_t)hdr_len);
    ...
    const char *body_start = find_http_body(hdr_buf, (uint32_t)hdr_len);
    if (!body_start) {
        mkt_close(&drv_conn);
        return HAL_ERROR;
    }

    uint32_t hdr_size = (uint32_t)(body_start - hdr_buf);
    uint32_t initial_body_len = (uint32_t)hdr_len - hdr_size;

    /* Parse Content-Length to know total body size */
    int32_t content_length = parse_content_length(hdr_buf, hdr_size);
    uint32_t total_body;
    if (content_length > 0) {
        total_body = (uint32_t)content_length;
    } else {
        /* No Content-Length — use what we already received */
        total_body = initial_body_len;
    }

    if (total_body == 0 || total_body > MAX_DRIVER_SIZE) {
        mkt_close(&drv_conn);
        hal_console_printf("[marketplace] Invalid body size: %u\n", total_body);
        return HAL_ERROR;
    }

    /* Allocate DMA buffer for the full driver binary */
    uint64_t phys_addr;
    void *drv_buf = hal_dma_alloc(total_body, &phys_addr);
    if (!drv_buf) {
        mkt_close(&drv_conn);
        return HAL_ERROR;
    }

    /* Copy the body bytes we already received */
    uint32_t received = initial_body_len;
    if (received > total_body) received = total_body;
    memcpy(drv_buf, body_start, received);

    /* Stream remaining body in chunks */
    while (received < total_body) {
        uint32_t remaining = total_body - received;
        uint32_t chunk_max = remaining < TCP_RX_BUF_SIZE ? remaining : TCP_RX_BUF_SIZE;

        int32_t chunk = mkt_recv(&drv_conn, (uint8_t *)drv_buf + received,
                                  chunk_max, 10000);
        if (chunk <= 0) {
            /* Connection closed or timeout — use what we have if mostly done */
            if (received > total_body / 2) {
                hal_console_printf("[marketplace] Partial download: %u/%u bytes\n",
                                   received, total_body);
                break;
            }
            hal_dma_free(drv_buf, total_body);
            mkt_close(&drv_conn);
            return HAL_TIMEOUT;
        }
...
/* ── Shared helper: build host string for HTTP Host header ── */
static void build_host_str(uint32_t ip, char *host_str)
{
#ifdef MARKETPLACE_USE_TLS
    /* Use hostname for TLS (SNI already handled) */
    const char *h = MARKETPLACE_HOST;
    char *p = host_str;
    while (*h) *p++ = *h++;
    *p = '\0';
#else
...
/* ── Shared helper: perform HTTP GET on fresh connection ── */
static int http_get(const char *path, char *response, uint32_t resp_max)
{
    ...
    int32_t resp_len = mkt_recv(&conn, response, resp_max - 1, 10000);
    mkt_close(&conn);
...
/* ── Shared helper: HTTP POST with JSON body on fresh connection ── */
                     ...
                     char *response, uint32_t resp_max)
{
    ...
    int32_t sent = mkt_send(&conn, request, (uint32_t)req_len);
    if (sent < 0) { mkt_close(&conn); return -1; }
