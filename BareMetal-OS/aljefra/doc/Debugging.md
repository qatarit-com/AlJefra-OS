...
Start a minimal QEMU AlJefra instance

	qemu-system-x86-64 -s -drive format=raw,file=aljefra_os.img
...
QEMU has a built in monitor to allow you to query the state of the VM. Running AlJefra via `./aljefra.sh run` in `AlJefra-OS` enables the monitor telnet port.
