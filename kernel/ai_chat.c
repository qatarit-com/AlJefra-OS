...
int ai_execute_action(const ai_action_t *action, char *output, int output_size)
{
    ...
    case ACTION_SYS_INFO: {
        ...
        safe_copy(output, "AlJefra OS v0.7.5\n", omax);
    ...
    case ACTION_SYS_UPDATE: {
        ...
        safe_append(output, "AlJefra OS v0.7.5 is the current version.\n", omax);
...
int ai_get_context(char *buf, int size)
{
    ...
    safe_copy(buf,
        ...
        "operating system.\n"
        "AlJefra OS v0.7.5 is a universal boot OS that runs on x86-64, "
        "ARM64, and RISC-V.\n\n",
        smax);
